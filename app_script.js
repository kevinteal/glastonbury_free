// JavaScript Document
var db = openDatabase('glastonbury_2016_bands_db_free', '1.0', 'bands db', 2 * 1024 * 1024);
var total_stages =0;
var day_arr=[];
var day_name_arr=[];
...
$(document).ready(function(e) {
	...
	setInterval(function(){
		...
		if(text=="Includes now playing section on home screen. Quickly see whos on and next on for each stage."){
			...
			$(".ad_box_main").css("background-image","url(imgs/home.png)");
		}
		}, 5000);
	...
	db.transaction(function (txs) {
		...
		txs.executeSql('select DISTINCT stage_rank, stage from '+_table_name_+' order by stage_rank ASC', [], function(txs, results){
						...
						for(var i = 0; i<=total_stages;i++){
									...
									set_up_lineup_page();
									$("#not_time").html("<center><a href='https://play.google.com/store/apps/details?id=com.DownloadClashFinder' target='_blank'>Download full verison from Google Play Store.</a> </center>");
								}
